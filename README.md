...
- 佣金运行批次持久化方案、受益人、有效业绩、基础费率、加速区间、封顶和争议；
- 服务端执行阶梯计算并分别返回毛佣金、应付佣金和是否触发封顶；
- 计算结果默认生成 100% 归属记录，并支持多人比例拆分；提交前强制校验受益人唯一、比例合计 100% 和分配金额守恒；
- 复核门禁校验方案事前审批、业绩数据锁定、合规检查及未决归属争议；
- 流程覆盖计算、提交、管理员审批、发放和部分/全额追索，禁止超额追索。
