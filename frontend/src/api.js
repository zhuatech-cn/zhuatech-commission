...
export const api={
  ...
  enterpriseControls:(params={})=>request('/api/enterprise/controls?'+query(params)),
  enterpriseSummary:(params={})=>request('/api/enterprise/summary?'+query(params)),
  enterpriseWorkbench:(params={})=>request('/api/enterprise/workbench?'+query(params)),
  periodStatus:(organizationCode,fiscalPeriod)=>request('/api/enterprise/period-status?'+query({organizationCode,fiscalPeriod})),
  setPeriodLock:data=>request('/api/admin/enterprise/period-lock',{method:'PUT',body:JSON.stringify(data)}),
  bulkSubmitEnterprise:(ids,remark)=>request('/api/enterprise/controls/bulk-submit',{method:'POST',body:JSON.stringify({ids,remark})}),
  bulkReviewEnterprise:(ids,decision,remark)=>request('/api/admin/enterprise/controls/bulk-review',{method:'POST',body:JSON.stringify({ids,decision,remark})}),
